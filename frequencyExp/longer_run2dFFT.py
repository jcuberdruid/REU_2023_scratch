...
outputDir = paths.dirBase+"fft_grey_64_640/"
...
def generate_fft_images(data, output_directory):
    ...
    # FFT on the entire 64x640 segment
    fft_data = np.fft.fft2(eeg_data)
    fft_data_magnitude = np.abs(fft_data)
    fft_data_log = np.log(fft_data_magnitude + 1)
    filename = f"S{subject}_r{run}_e{epoch}_{condition}.png"
