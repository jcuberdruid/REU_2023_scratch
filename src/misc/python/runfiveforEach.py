...
def runSubject(testingSubjects, subjects): # testing subject, and training subject
    #exclude = testingSubjects + [88, 89, 92, 100, 104] 
    #subjects = [x for x in range(1, 5) if x not in exclude]
    #random.shuffle(subjects)
...
runSubject([int(sys.argv[1])], [int(sys.argv[2])])
