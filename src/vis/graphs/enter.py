            ...
            if filename.startswith('output'):
                ...
                with open(file_path) as file:
                    ...
                    file_df = pd.DataFrame({
                        ...
                        'Model File Path': model_file_path  # add this line
                    })
...
# Get the model signature of the best model
best_model_signature = grouped_data.iloc[-1]['Model Signature']

# Get the corresponding model file path from the data_df DataFrame
best_model_file_path = data_df[data_df['Model Signature'] == best_model_signature]['Model File Path'].values[0]

# Load and print the contents of the JSON file
with open(best_model_file_path, 'r') as file:
    best_model_data = json.load(file)
print(json.dumps(best_model_data, indent=4))  # Pretty print the JSON data
