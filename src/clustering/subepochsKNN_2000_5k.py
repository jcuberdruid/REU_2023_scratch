...
def cluster(targetSubject, subjectObject, video_data, data_npy, annotations_arr, start_clusters=15):
    ...
    nbrs = NearestNeighbors(n_neighbors=4, algorithm='brute', metric='minkowski')
    ...
    for index, x in enumerate(annotations_arr):
        thisDict = {'subject': int(x['subject']), 'epoch': int(x['epoch']), 'distance': distances[index], 'index':int(x['index'])}
        clusteredEpochs.append(thisDict)

    clusteredEpochs = sorted(clusteredEpochs, key=lambda x: x["distance"])
