...
for i in range(n_videos):
    video_vectors = video_data_flattened[i * n_frames: (i + 1) * n_frames]
    bmu_positions = np.array([som.winner(vector)[-1] for vector in video_vectors])
...
color_map = plt.cm.get_cmap('tab10', len(np.unique(cluster_labels)))
...
for i, video_vector in enumerate(video_data_flattened):
    winner = som.winner(video_vector)
    plt.plot(winner[0] + 0.5, winner[1] + 0.5, 'o', markerfacecolor=color_map(cluster_labels[i]), markersize=8, markeredgecolor='k')
